...
//Used to select between upper and lower uint8_t of command register
#define LOWER true
#define UPPER false
...
bool VCNL4040::begin() {
    ...
    if(isConnected() == false){
        printf("VCNL4040 - Not connected\n");
        return false;
    }

    if(getID() != 0x0186){
        printf("VCNL4040 - get ID failure\n");
        return false;
...
bool VCNL4040::isConnected(void) {

    return qwiic.pingDevice(VCNL4040_ADDR);
...
{
    if(dutyValue > 320 - 1) 
        dutyValue = VCNL4040_PS_DUTY_320;
    else if(dutyValue > 160 - 1) 
        dutyValue = VCNL4040_PS_DUTY_160;
    else if(dutyValue > 80 - 1) 
        dutyValue = VCNL4040_PS_DUTY_80;
    else 
        dutyValue = VCNL4040_PS_DUTY_40;
...
{
    if(timeValue > 640 - 1) 
        timeValue = VCNL4040_ALS_IT_640MS;
    else if(timeValue > 320 - 1) 
        timeValue = VCNL4040_ALS_IT_320MS;
    else if(timeValue > 160 - 1) 
        timeValue = VCNL4040_ALS_IT_160MS;
    else 
        timeValue = VCNL4040_ALS_IT_80MS;
...
void VCNL4040::setLEDCurrent(uint8_t currentValue)
{
	if(currentValue > 200 - 1) 
        currentValue = VCNL4040_LED_200MA;
	else if(currentValue > 180 - 1) 
        currentValue = VCNL4040_LED_180MA;
	else if(currentValue > 160 - 1) 
        currentValue = VCNL4040_LED_160MA;
	else if(currentValue > 140 - 1) 
        currentValue = VCNL4040_LED_140MA;
	else if(currentValue > 120 - 1) 
        currentValue = VCNL4040_LED_120MA;
	else if(currentValue > 100 - 1) 
        currentValue = VCNL4040_LED_100MA;
	else if(currentValue > 75 - 1) 
        currentValue = VCNL4040_LED_75MA;
	else 
        currentValue = VCNL4040_LED_50MA;
...
//Reads two consecutive uint8_ts from a given 'command code' location
uint16_t VCNL4040::readCommand(uint8_t commandCode)
{

    uint16_t value;
    int rc = qwiic.readRegisterInt16(VCNL4040_ADDR, commandCode, &value);
...
//Write two uint8_ts to a given command code location (8 bits)
bool VCNL4040::writeCommand(uint8_t commandCode, uint16_t value)
{

    return qwiic.writeRegisterInt16(VCNL4040_ADDR, commandCode, value);

}

//Given a command code (address) write to the lower uint8_t without affecting the upper uint8_t
...
//Given a command code (address) write to the upper uint8_t without affecting the lower uint8_t
...
//Given a command code (address) read the lower uint8_t
...
//Given a command code (address) read the upper uint8_t
...
//commandHeight is used to select between the upper or lower uint8_t of command register
//Example:
//Write dutyValue into PS_CONF1, lower uint8_t, using the Duty_Mask
//bitMask(VCNL4040_PS_CONF1, LOWER, VCNL4040_PS_DUTY_MASK, dutyValue);
void VCNL4040::bitMask(uint8_t commandAddress, bool commandHeight, uint8_t mask, uint8_t thing)
{
    ...
    if(commandHeight == LOWER) 
        registerContents = readCommandLower(commandAddress);
    else 
        registerContents = readCommandUpper(commandAddress);
